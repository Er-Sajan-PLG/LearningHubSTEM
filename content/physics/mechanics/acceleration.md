...
status: draft
definition: >-
  The rate of change of velocity with respect to time; a vector quantity. Acceleration occurs
  whenever an object's velocity changes — whether in magnitude, direction, or both.
symbol: a
...
common_misconceptions:
  ...
  - Acceleration always means speeding up (slowing down and changing direction are also
    acceleration).
  - If velocity is zero, acceleration must be zero (an object can have zero velocity and
    non-zero acceleration, e.g., at the top of a vertical throw).
  - If speed is constant, acceleration must be zero (changing direction at constant speed is
    still acceleration).
learning_objectives:
